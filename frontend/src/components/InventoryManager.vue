...
<template>
  <div class="bg-slate-50 rounded-lg border border-slate-200 p-6">
    <h3 class="text-xs uppercase tracking-wide text-slate-400 font-bold mb-4 border-b border-slate-200 pb-2">{{ t('inventory.title') }}</h3>
    ...
    <div v-else class="flex flex-col gap-4">
      ...
      <div class="flex flex-col gap-4">
        ...
        >
          ...
          <span v-else>{{ operation === StockOperationsType.ADD ? t('inventory.addStock') : t('inventory.removeStock') }}</span>
        </button>
      </div>
    </div>
  </div>
